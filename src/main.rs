use sss::gao;
// use sss::{mds, og_matrix, reconstruct_bytes, reconstruct_leak, share_bytes, share_leaks};
use sss::Fq;
...
fn main() {
    ...
    let n = 5;
    let errors = 1;
    let vec = DMatrix::from_vec(5, 1, vec![Fq::from(0), Fq::from(0), Fq::from(0), Fq::from(4), Fq::from(0)]);
